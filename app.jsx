...
function calcFormat(n) {
  if (n == null || isNaN(n) || !isFinite(n)) return "Hata";
  const rounded = Math.round(n * 1e9) / 1e9; // kayan nokta artıklarını temizle
  return rounded.toLocaleString("tr-TR", { maximumFractionDigits: 9 });
}
function CalculatorPopover({ onClose }) {
  const [display, setDisplay] = useStateA("0");
  const [prev, setPrev] = useStateA(null);
  const [op, setOp] = useStateA(null);
  const [waiting, setWaiting] = useStateA(false);
  const [expr, setExpr] = useStateA("");

  const curNum = () => parseFloat(display.replace(/\./g, "").replace(",", ".")) || 0;
  ...
  const clearAll = () => { setDisplay("0"); setPrev(null); setOp(null); setWaiting(false); setExpr(""); };
  const backspace = () => {
    if (waiting) return;
    setDisplay(display.length > 1 ? display.slice(0, -1) : "0");
  };
  const toggleSign = () => { if (display !== "0") setDisplay(display.startsWith("-") ? display.slice(1) : "-" + display); };
  const percent = () => setDisplay(calcFormat(curNum() / 100));
  ...
  const chooseOp = (nextOp) => {
    ...
    if (prev != null && !waiting) {
      ...
      setExpr(calcFormat(result) + " " + nextOp);
    } else {
      setPrev(cur);
  ...
  const onKey = (e) => {
    ...
    if (e.key === "," || e.key === ".") { inputComma(); return; }
  ...
  const Key = ({ label, onClick, kind }) => (
    <button
      type="button"
      onClick={onClick}
      style={{
        height: 42, borderRadius: 9, border: "1px solid var(--border)", fontFamily: "inherit",
        fontSize: kind === "digit" ? 15 : 14, fontWeight: 600, cursor: "pointer",
        background: kind === "op" ? "var(--accent)" : kind === "fn" ? "var(--bg-elev-3)" : "var(--bg-elev-2)",
        color: kind === "op" ? "var(--accent-fg)" : "var(--fg)",
      }}
    >{label}</button>
  );
  ...
  return (
    <>
      ...
      <div className="bell-popover" style={{ width: 268, padding: 12 }}>
        <div style={{ padding: "8px 6px 12px", textAlign: "right" }}>
          <div style={{ fontSize: 11, color: "var(--fg-4)", minHeight: 14, marginBottom: 4 }}>{expr}</div>
          <div className="mono" style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.01em", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{display}</div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 7 }}>
          <Key label="C" kind="fn" onClick={clearAll} />
          <Key label="±" kind="fn" onClick={toggleSign} />
          <Key label="%" kind="fn" onClick={percent} />
          <Key label="÷" kind="op" onClick={() => chooseOp("÷")} />

          <Key label="7" kind="digit" onClick={() => inputDigit("7")} />
          <Key label="8" kind="digit" onClick={() => inputDigit("8")} />
          <Key label="9" kind="digit" onClick={() => inputDigit("9")} />
          <Key label="×" kind="op" onClick={() => chooseOp("×")} />

          <Key label="4" kind="digit" onClick={() => inputDigit("4")} />
          <Key label="5" kind="digit" onClick={() => inputDigit("5")} />
          <Key label="6" kind="digit" onClick={() => inputDigit("6")} />
          <Key label="−" kind="op" onClick={() => chooseOp("−")} />

          <Key label="1" kind="digit" onClick={() => inputDigit("1")} />
          <Key label="2" kind="digit" onClick={() => inputDigit("2")} />
          <Key label="3" kind="digit" onClick={() => inputDigit("3")} />
          <Key label="+" kind="op" onClick={() => chooseOp("+")} />

          <span />
          <Key label="0" kind="digit" onClick={() => inputDigit("0")} />
          <Key label="," kind="digit" onClick={inputComma} />
          <button
            type="button" onClick={equals}
            style={{ height: 42, borderRadius: 9, border: "none", fontFamily: "inherit", fontSize: 16, fontWeight: 700, cursor: "pointer", background: "var(--fg)", color: "var(--bg)" }}
          >=</button>

          <button
            type="button" onClick={backspace}
            style={{ gridColumn: "1 / -1", height: 36, marginTop: 2, borderRadius: 9, border: "1px solid var(--border)", fontFamily: "inherit", fontSize: 13, fontWeight: 600, cursor: "pointer", background: "transparent", color: "var(--fg-3)", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
          ><Icon name="arrowLeft" size={13} />Sil</button>
        </div>
      </div>
    </>
  );
...
function TopBar({ onAddTransaction, onBulkAdd, ctx }) {
  ...
  const [calcOpen, setCalcOpen] = useStateA(false);
    ...
    <header className="topbar">
      ...
      <div className="topbar-actions">
        <div className="bell-wrap">
          <button className="icon-btn" onClick={() => setCalcOpen((v) => !v)} title="Hesap makinesi">
